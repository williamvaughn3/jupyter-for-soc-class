...
> An outstanding resource to use is everything.curl
[EverythingCurl Link](https://everything.curl.dev)
...
# The prior curl command had an HTTP header of Set-Cookie. Save to a file:
curl -D - http://127.0.0.1 -c cookie.txt
# JSON
curl http://127.0.0.1/cache | python -m json.tool
# Attempt to POST deleteone=files to delete one file:
curl -s http://127.0.0.1/cache --cookie cookie.txt -d deleteone=files
# response: {"data":"Curl Not Allowed","request":false}
# Change the useragent string via the following command:
